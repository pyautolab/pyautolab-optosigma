...
class StageControlManager(api.widgets.Manager):
    def __init__(self, device: StageController):
        ...
        self._t_btn_up = api.qt_helpers.create_tool_button(
        ...
        self._t_btn_down = api.qt_helpers.create_tool_button(
        ...
        self._t_btn_emergency_stop = api.qt_helpers.create_tool_button(
            fixed_width=50,
            fixed_height=50,
        ...
        self._p_button_fix_zero = api.qt_helpers.create_push_button(clicked=self.fix_zero, text="Fix Zero")
        self._p_button_move_machine_zero = api.qt_helpers.create_push_button(
            clicked=self.move_to_machine_zero, text="Move Machine Zero-Point"
        )
        self._p_button_set_speed = api.qt_helpers.create_push_button(
            clicked=self.set_stage_speed, fixed_width=100, text="Set"
        )

        # timer
        self.timer_measure_position = api.qt_helpers.create_timer(parent=self, timeout=self.measure_position)

        # setup
        self._int_slider.range = (1, 50000)

        # setup layout
        v_layout_control = api.qt_helpers.create_v_box_layout(
            [self._t_btn_up, self._t_btn_emergency_stop, self._t_btn_down]
        )
        v_layout_control.setAlignment(Qt.AlignmentFlag.AlignHCenter)
        ...
        group_position.setLayout(
            api.qt_helpers.create_h_box_layout([api.qt_helpers.add_unit(self._lcd_position, "μm")])
        )

        group_origin = QGroupBox("Origin")
        group_origin.setLayout(
            api.qt_helpers.create_v_box_layout([self._p_button_fix_zero, self._p_button_move_machine_zero])
        )

        v_layout_speed = QVBoxLayout()
        v_layout_speed.addWidget(api.qt_helpers.add_unit(self._int_slider, "μm/msec"))
    ...
    @Slot()
    @api.qt_helpers.popup_exception_message(exception=SerialException)
